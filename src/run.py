...
block_size = 128
text = codecs.open(args.pretrain_corpus_path,"r",encoding='utf-8').read()
...
if args.function == 'pretrain':
    ...
    warmup_tokens=512*20, final_tokens=2*len(pretrain_dataset)*block_size, num_workers=0, ckpt_path=args.writing_params_path)
    ...
    max_epochs_ = 75 if args.reading_params_path is None else 10
    if args.reading_params_path is not None:
        model.load_state_dict(torch.load(args.reading_params_path))

    tconf = trainer.TrainerConfig(max_epochs=max_epochs_, batch_size=256, learning_rate=6e-4, lr_decay=True, \
    warmup_tokens=512*20, final_tokens=2*len(finetune_dataset)*block_size, num_workers=0, ckpt_path=args.writing_params_path)
